...
DO $$ BEGIN CREATE TYPE platform_lobby_status AS ENUM (
    'waiting',
    'starting',
    'in_game',
    'post_game',
...
END $$;
DO $$ BEGIN CREATE TYPE platform_lobby_game AS ENUM ('quiz', 'bilionar', 'higher_lower');
EXCEPTION
WHEN duplicate_object THEN null;
END $$;
DO $$ BEGIN CREATE TYPE lobby_member_role AS ENUM ('host', 'member');
EXCEPTION
WHEN duplicate_object THEN null;
END $$;
DO $$ BEGIN CREATE TYPE lobby_member_state AS ENUM ('in_lobby', 'in_game', 'disconnected', 'left');
EXCEPTION
WHEN duplicate_object THEN null;
END $$;
DO $$ BEGIN CREATE TYPE platform_match_type AS ENUM ('quiz', 'bilionar', 'higher_lower');
EXCEPTION
WHEN duplicate_object THEN null;
END $$;
DO $$ BEGIN CREATE TYPE platform_match_status AS ENUM ('preparing', 'live', 'finished', 'aborted');
EXCEPTION
WHEN duplicate_object THEN null;
END $$;
DO $$ BEGIN CREATE TYPE match_end_reason AS ENUM (
    'normal',
    'forfeit',
    'host_left',
    'timeout',
...
WHEN duplicate_object THEN null;
END $$;
DO $$ BEGIN CREATE TYPE match_player_state AS ENUM (
    'active',
    'eliminated',
    'left',
    'disconnected',
...
ALTER TABLE platform_lobbies
ADD COLUMN status platform_lobby_status DEFAULT 'waiting';
ALTER TABLE platform_lobbies DROP COLUMN IF EXISTS selected_game;
ALTER TABLE platform_lobbies
ADD COLUMN selected_game platform_lobby_game DEFAULT 'quiz';
ALTER TABLE platform_lobbies
ADD COLUMN IF NOT EXISTS settings JSONB DEFAULT '{}'::jsonb;
ALTER TABLE platform_lobbies
ADD COLUMN IF NOT EXISTS active_match_id UUID DEFAULT NULL;
ALTER TABLE platform_lobbies
ADD COLUMN IF NOT EXISTS version INT DEFAULT 1;
ALTER TABLE platform_lobbies
ADD COLUMN IF NOT EXISTS closed_at TIMESTAMPTZ DEFAULT NULL;
ALTER TABLE platform_lobbies
ADD COLUMN IF NOT EXISTS lobby_ttl_expires_at TIMESTAMPTZ DEFAULT NULL;
...
CREATE TABLE platform_matches (
    ...
    SET NULL,
        winner_user_id UUID REFERENCES profiles(id) ON DELETE
    SET NULL,
...
BEGIN;
ALTER TABLE platform_lobbies
ADD CONSTRAINT fk_active_match FOREIGN KEY (active_match_id) REFERENCES platform_matches(id) ON DELETE
SET NULL;
COMMIT;
...
CREATE OR REPLACE FUNCTION transfer_host_on_leave() RETURNS TRIGGER AS $$
DECLARE new_host UUID;
BEGIN IF OLD.role = 'host'
AND NEW.state = 'left' THEN -- Find the oldest remaining member who is not 'left'
...
SET role = 'host'
WHERE lobby_id = NEW.lobby_id
    AND user_id = new_host;
UPDATE platform_lobbies
SET host_user_id = new_host
WHERE id = NEW.lobby_id;
ELSE -- No one left, close lobby
UPDATE platform_lobbies
SET status = 'closed',
    closed_at = now()
WHERE id = NEW.lobby_id;
...
AFTER
UPDATE OF state ON lobby_members FOR EACH ROW
    WHEN (
        OLD.state != 'left'
        AND NEW.state = 'left'
...
CREATE POLICY "Public profiles are viewable by everyone." ON lobby_members FOR
SELECT USING (true);
DROP POLICY IF EXISTS "Enable insert access for all" ON lobby_members;
CREATE POLICY "Enable insert access for all" ON lobby_members FOR
INSERT WITH CHECK (true);
DROP POLICY IF EXISTS "Users can update their own lobby membership." ON lobby_members;
CREATE POLICY "Enable update access for all" ON lobby_members FOR
UPDATE USING (true);
DROP POLICY IF EXISTS "Enable read access for all" ON platform_matches;
CREATE POLICY "Enable read access for all" ON platform_matches FOR
SELECT USING (true);
DROP POLICY IF EXISTS "Enable insert access for all" ON platform_matches;
CREATE POLICY "Enable insert access for all" ON platform_matches FOR
INSERT WITH CHECK (true);
DROP POLICY IF EXISTS "Enable update access for all" ON platform_matches;
CREATE POLICY "Enable update access for all" ON platform_matches FOR
UPDATE USING (true);
DROP POLICY IF EXISTS "Enable read access for all" ON match_players;
CREATE POLICY "Enable read access for all" ON match_players FOR
SELECT USING (true);
DROP POLICY IF EXISTS "Enable insert access for all" ON match_players;
CREATE POLICY "Enable insert access for all" ON match_players FOR
INSERT WITH CHECK (true);
