...
// ─── QUESTIONS ───────────────────────────────────────────────────────────────
const QUESTIONS = [
  { q: 'Aké je hlavné mesto Slovenska?', answers: ['Bratislava', 'Košice', 'Žilina', 'Prešov'], correct: 0 },
  { q: 'Koľko planét má Slnečná sústava?', answers: ['7', '8', '9', '10'], correct: 1 },
  { q: 'Kto napísal Romea a Júliu?', answers: ['Dickens', 'Shakespeare', 'Tolstoj', 'Hemingway'], correct: 1 },
  { q: 'V ktorom roku padol Berlínsky múr?', answers: ['1987', '1988', '1989', '1990'], correct: 2 },
  { q: 'Aký je chemický vzorec vody?', answers: ['CO2', 'H2O', 'NaCl', 'O2'], correct: 1 },
  { q: 'Ktorá krajina má najviac obyvateľov?', answers: ['USA', 'India', 'Čína', 'Rusko'], correct: 1 },
  { q: 'Koľko strán má trojuholník?', answers: ['2', '3', '4', '5'], correct: 1 },
  { q: 'Aký je najväčší oceán?', answers: ['Atlantický', 'Indický', 'Tichý', 'Severný ľadový'], correct: 2 },
  { q: 'Kto namaľoval Monu Lízu?', answers: ['Picasso', 'Van Gogh', 'Da Vinci', 'Michelangelo'], correct: 2 },
  { q: 'V akom športe sa používa puk?', answers: ['Futbal', 'Hokej', 'Tenis', 'Baseball'], correct: 1 },
  { q: 'Koľko minút má hodina?', answers: ['30', '45', '60', '90'], correct: 2 },
  { q: 'Ktoré zviera je najrýchlejšie?', answers: ['Lev', 'Gepard', 'Kôň', 'Antilopa'], correct: 1 },
  { q: 'Aké je hlavné mesto Francúzska?', answers: ['Lyon', 'Marseille', 'Paríž', 'Nice'], correct: 2 },
  { q: 'Koľko kostí má dospelý človek?', answers: ['106', '156', '206', '256'], correct: 2 },
  { q: 'Ktorý prvok má symbol Fe?', answers: ['Fluór', 'Fosfor', 'Železo', 'Francium'], correct: 2 },
  { q: 'V ktorom roku sa začala 2. svetová vojna?', answers: ['1935', '1937', '1939', '1941'], correct: 2 },
  { q: 'Aký je najvyšší vrch sveta?', answers: ['K2', 'Everest', 'Kilimandžáro', 'Mont Blanc'], correct: 1 },
  { q: 'Koľko dní má prestupný rok?', answers: ['364', '365', '366', '367'], correct: 2 },
  { q: 'Ktorý plyn dýchame?', answers: ['Dusík', 'Kyslík', 'CO2', 'Hélium'], correct: 1 },
  { q: 'Kde sa konali OH 2024?', answers: ['Tokyo', 'Paríž', 'Los Angeles', 'Londýn'], correct: 1 },
  { q: 'Aké je hlavné mesto Japonska?', answers: ['Osaka', 'Tokio', 'Kjóto', 'Hirošima'], correct: 1 },
  { q: 'Koľko chromozómov má človek?', answers: ['23', '44', '46', '48'], correct: 2 },
  { q: 'Kto vynašiel žiarovku?', answers: ['Tesla', 'Edison', 'Bell', 'Watt'], correct: 1 },
  { q: 'V ktorom roku pristál človek na Mesiaci?', answers: ['1965', '1967', '1969', '1971'], correct: 2 },
  { q: 'Aká je najdlhšia rieka na svete?', answers: ['Amazonka', 'Níl', 'Yangtze', 'Mississippi'], correct: 1 },
  { q: 'Koľko hráčov má futbalový tím?', answers: ['9', '10', '11', '12'], correct: 2 },
  { q: 'Ktorý kontinent je najväčší?', answers: ['Afrika', 'Ázia', 'Európa', 'Sev. Amerika'], correct: 1 },
  { q: 'Aký je symbol zlata?', answers: ['Ag', 'Au', 'Zn', 'Cu'], correct: 1 },
  { q: 'Kto napísal Hamleta?', answers: ['Molière', 'Shakespeare', 'Goethe', 'Čechov'], correct: 1 },
  { q: 'Koľko zubov má dospelý človek?', answers: ['28', '30', '32', '34'], correct: 2 },
];
...
class Bot {
  ...
  getAnswer(correctIdx) {
    ...
    let wrong;
    do { wrong = Math.floor(Math.random() * 4); } while (wrong === correctIdx);
...
export class QuizDuelGame {
  constructor(canvas, user, { onBack }) {
    ...
    // State
    this.phase = 'menu'; // menu | countdown | question | reveal | result
    this.round = 0;
    this.myScore = 0;
    this.botScore = 0;
    this.myAnswer = null; // 0-3 or null
    this.botAnswer = null;
  ...
  _startGame() {
    this.questions = shuffle(QUESTIONS).slice(0, TOTAL_ROUNDS);
  ...
  _onMove(e) {
    const p = this._pos(e);
    for (let i = 0; i < 4; i++) {
      gsap.to(this.anim.ansH, { [i]: this._hit(p, this.hits[`a${i}`]) ? 1 : 0, duration: 0.15 });
    }
    for (let i = 0; i < 3; i++) {
      gsap.to(this.anim.diffH, { [i]: this._hit(p, this.hits[`d${i}`]) ? 1 : 0, duration: 0.15 });
    }
  ...
  _drawMenu() {
    ...
    ctx.textAlign = 'center'; ctx.textBaseline = 'middle';
    ctx.shadowColor = C.green; ctx.shadowBlur = 20;
    ctx.fillStyle = C.green;
    ctx.fillText('KVÍZ DUEL', cx, titleY);
    ctx.shadowBlur = 0;

    ctx.font = `500 ${mobile ? 12 : 15}px Inter, system-ui, sans-serif`;
    ctx.fillStyle = C.muted;
    ...
    ctx.fillText('Obtiažnosť BOTa', cx, dsy - 16);
    ...
    for (let i = 0; i < 3; i++) {
      ...
      ctx.fillStyle = active ? hex2rgba(diffCols[i], 0.2) : `rgba(255,255,255,${0.03 + hover * 0.05})`;
      ctx.fill();
      rr(ctx, dx, dsy, dbw, dbh, 12);
      ctx.strokeStyle = active ? diffCols[i] : `rgba(255,255,255,${0.1 + hover * 0.1})`;
    ...
    this.hits.play = pb;
    ctx.shadowColor = C.green; ctx.shadowBlur = 12 + anim.playH * 20;
    const g = ctx.createLinearGradient(pb.x, pb.y, pb.x, pb.y + pbh);
    g.addColorStop(0, anim.playH ? C.greenL : C.green);
    g.addColorStop(1, anim.playH ? C.green : '#15803d');
  ...
  _drawQuestion() {
    ...
    if (pct > 0) {
      ...
      ctx.fillStyle = pct > 0.3 ? C.green : C.red; ctx.fill();
    }
    ...
    for (let i = 0; i < 4; i++) {
      ...
      if (revealing) {
        if (isCorrect) {
          ...
          bgColor = 'rgba(255,255,255,0.02)'; borderColor = 'rgba(255,255,255,0.05)'; textColor = C.dim;
        }
      } else if (isMyPick) {
        bgColor = hex2rgba(ansColors[i], 0.2); borderColor = ansColors[i]; textColor = '#fff';
      } else {
        bgColor = `rgba(255,255,255,${0.03 + hover * 0.05})`;
        borderColor = `rgba(255,255,255,${0.08 + hover * 0.15})`;
    ...
    // Bot status
    const bsY = astartY + 2 * (abh + agap) + 15;
    ctx.font = `500 ${mobile ? 11 : 13}px Inter, system-ui, sans-serif`;
    if (this.phase === 'reveal') {
      const botCorrect = this.botAnswer === q.correct;
      ctx.fillStyle = botCorrect ? C.green : C.red;
  ...
  _drawScoreboard() {
    ...
    ctx.textAlign = 'right';
    ctx.font = `700 ${mobile ? 13 : 16}px Inter, system-ui, sans-serif`;
    ctx.fillStyle = C.greenL;
    ctx.fillText('Ty', W/2 - 45, y);
    ctx.font = `900 ${mobile ? 18 : 22}px Inter, system-ui, sans-serif`;
    ctx.fillStyle = '#fff';
  ...
  _drawResult() {
    ...
    const won = this.myScore > this.botScore;
    const tied = this.myScore === this.botScore;
    ...
    ctx.shadowColor = borderC; ctx.shadowBlur = 20;
    ...
    ctx.fillStyle = C.greenL;
    ctx.fillText('Ty', cx - 70, cy - 20);
    ctx.fillStyle = C.purple;
    ctx.fillText('BOT', cx + 70, cy - 20);

    ctx.font = '900 52px Inter, system-ui, sans-serif';
    ctx.fillStyle = '#fff';
    ctx.fillText(`${this.myScore}`, cx - 70, cy + 30);
    ctx.fillText(`${this.botScore}`, cx + 70, cy + 30);
